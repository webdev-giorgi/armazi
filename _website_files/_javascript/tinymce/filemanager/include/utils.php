...
function create_img($imgfile, $imgthumb, $newwidth, $newheight = "", $option = "crop")
{
    ...
    if ($mime == 'image/webp') {
        // Handle WebP image using GD
        $source = imagecreatefromwebp($imgfile);

        if ($newheight == "") {
            $newheight = (imagesy($source) / imagesx($source)) * $newwidth;
        }

        $thumb = imagecreatetruecolor($newwidth, $newheight);
        imagecopyresampled($thumb, $source, 0, 0, 0, 0, $newwidth, $newheight, imagesx($source), imagesy($source));
        imagewebp($thumb, $imgthumb, 80);  // Save WebP with quality

        imagedestroy($source);
        imagedestroy($thumb);
        $result = true;
    } else {
        // Handle non-WebP images using imageLib
        if (image_check_memory_usage($imgfile, $newwidth, $newheight)) {
            require_once('php_image_magician.php');
            $magicianObj = new imageLib($imgfile);
