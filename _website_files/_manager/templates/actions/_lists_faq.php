...
function _build_lists($parent, $order_by, $start = -1,$per_page)
{
    ...
    foreach ($results AS $result):
        $is_first = ($count == 1);
        $is_last = ($count >= $total);
        $arrow_up = $is_first ? '<img src="_manager/img/none.png" width="9" height="9" style="float:left;" />' : '<a href="javascript:pos(\'up\',\''.$result['id'].'\')"><img src="_manager/img/buttons/icon_arrow_up_' . ($result['level'] > 1 ? 1 : 2) . '.png" class="star" title="' .  a('tt.moveup') . '" width="9" height="9" /></a>';
        $arrow_down = $is_last ? '<img src="_manager/img/none.png" width="9" height="9" style="float:left;" />' : '<a href="javascript:pos(\'down\',\''.$result['id'].'\')"><img src="_manager/img/buttons/icon_arrow_down_' . ($result['level'] > 1 ? 1 : 2) . '.png" class="star" title="' .  a('tt.movedn') . '" width="9" height="9" /></a>';
		$arrows = ($order_by == 'position') ? $arrow_up . $arrow_down : '<img src="_manager/img/table.png" width="16" height="16" alt="" />';
        $edit_link = ahref(array($route[0], 'edit', $result["id"]));
        $count++;
        $pad = NULL;
...
?>
					<div class="<?php echo $class;?> fix" id="div<?php echo $result["id"];?>" >
						...
						<div class="name" style="float:left;width:560px;"><a href="<?php echo ahref(array($route[0], 'edit', $result["id"]));?>" style="width:100%; background:none;"><span class="star" title="<?php echo a('ql.edit');?>"><?php echo $pad . $result["title"];?>&nbsp;</a></span></div>									
